...
static class ProjectsQuery
{
    ...
    {
        return projects.SelectMany(p => p.Payments).Where(x => x.Date >= startDate && x.Date <= endDate).Select(x => new PaymentWrapper(x));
    }
    ...
    public static decimal CalculateMonthProfitForCurrency(DateTime date, string currency)
    {
        ...
        var paymentVMs = currencyProjects.SelectMany(p => p.Payments).Where(x => x.Date.Month == date.Month && x.Date.Year == date.Year).Select(p => new PaymentWrapper(p));
